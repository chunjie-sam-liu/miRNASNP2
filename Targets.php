...
<?function pageButton($count,$page,$pages, $sql){
	...
	<div style="magin:10px;">
		<ul class="pager" style="list-style-type: none">
			...
			<!--- <?$sql = explode('limit',$sql)[0];echo $sql?>--->
...
<?function show(){?>
	<?
	$sql = "SELECT * FROM table9_mir_tarrget_num_info";
	...
	<table class="table table-hover table-bordered tablesorter">
		...
		<tbody>
		<?while($thisline=mysql_fetch_array($result)){?>
			<tr>
				<td>
					<a href="http://www.mirbase.org/cgi-bin/query.pl?terms=<? echo $thisline["miRNA"] ?>" target="_blank">
						<? echo $thisline["miRNA"]; ?>
					</a>
				</td>
...
<!--	//for gain-->
<?function gain($gene,$mirna,$snp,$gene_expression,$mirna_expression){?>
	<?
	//condition
	$sql = "select * from seed_target_gain_with_base where ";
...
<!--	//for loss-->
<?function loss($gene,$mirna,$snp,$gene_expression,$mirna_expression, $validate, $negative){?>
	<?
	//condition
	$sql = "select * from seed_target_loss_with_base where ";
	...
	<table class="table table-hover table-bordered tablesorter" style="font-size: 14px;">
		<thead><tr class="info">
			...
			<th><span class="glyphicon glyphicon-question-sign green" data-toggle="tooltip" title="miRNA-gene validated by experiment" data-trigger="hover" data-placement="bottom"></span></th>
			<th>Cor.&nbsp;<span class="glyphicon glyphicon-question-sign red" data-toggle="popover" data-container="body" data-content="The expression correlation value between miRNA and gene is calculated based on expression of pairwise samples, this column is the most negatively correlated number; click number to show details." data-trigger="hover" data-placement="bottom"></span></th>
			<th style="text-align: center">&Delta;&Delta;G&nbsp;<span class="glyphicon glyphicon-question-sign red" data-toggle="popover" data-container="body" data-content="Energy change (kcal/mol)" data-trigger="hover" data-placement="bottom"></span></th>
			<th style="text-align: center">SNP-miRNA/target duplexes </th>
			<th style="text-align: center">Effect</th>
		</tr></thead>
...
<!--	form for search-->
			...
			<form class="form-horizontal" role="form" name="mirnaSearch2" method="get" action="Targets.php">
				...
				<div class="form-group has-error ">
					<div class="checkbox col-sm-offset-3 col-sm-4" data-triggle="hover" data-placement="right" data-original-title="only for loss">
						<label>
							<input type="checkbox" name="validate" value="1" class="disable-gain"><strong>miRNA:target experimental validated</strong>
						</label>
				...
				<div class="form-group has-error ">
					<div class="checkbox col-sm-offset-3 col-sm-6" data-triggle="hover" data-placement="right" data-original-title="only for loss">
						<label>
							<input type="checkbox" name="negative" value="1" class="disable-gain"><strong>miRNA expression negatively correlate with gene expression</strong>
						</label>
						<span class="glyphicon glyphicon-question-sign"  data-toggle="popover" data-container="body" data-content="Not available for Gain option, because there are no validated targets and expression correlation data for SNP type 3'UTR and miRNA" data-trigger="hover" data-placement="right"></span>
					</div>
				</div>
	...
	<?
	if($gainorlost == "gain"){
		gain($gene,$mirna,$snp,$gene_expression,$mirna_expression);
	}
	elseif($gainorlost == "loss"){
		loss($gene,$mirna,$snp,$gene_expression,$mirna_expression, $validate, $negative);
	}
	else{
		show();
	}
